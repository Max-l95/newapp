...
const Ingredienti = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await apiClient.get('/ingredienti');
    ...
    const handleAddCategory = async (event) => {
        ...
        try {
            const response = await apiClient.create('/ingredienti/add', {
    ...
    const handleEditCategory = async (event) => {
        event.preventDefault();
        try {
            const response = await apiClient.update('/ingredienti/edit', {
                id: editId,
    ...
    const confirmMultipleDelete = async () => {
        ...
        try {
            const response = await apiClient.delete('/ingredienti/delete', { ids: idsToDelete });
            if (response.success) {
                fetchData();
                setSelectedRows([]);
                toggleDelete();
    ...
    document.title = "Ingredienti | DgnsDesk";
    ...
    return (
        <React.Fragment>
                ...
                <Container fluid>
                    <Row>
                        <Col lg={12}>
                            <Card>
                                <CardHeader>
                                    <h4 className='card-title mb-0'>Ingredienti</h4>
                                </CardHeader>
                                <CardBody>
            ...
            <Modal isOpen={modalList} toggle={toggleList} centered>
                <ModalHeader className="bg-light p-3" toggle={toggleList}>Aggiungi Ingrediente</ModalHeader>
                <form className="tablelist-form" onSubmit={handleAddCategory}>
                    ...
                    <ModalFooter>
                        <div className="hstack gap-2 justify-content-end">
                            ...
                            <button type="submit" className="btn btn-success" id="add-btn">Aggiungi ingredientie</button>
                        </div>
                    </ModalFooter>
                </form>
            </Modal>
            ...
            <Modal isOpen={editModal} toggle={toggleEditModal} centered>
                <ModalHeader className="bg-light p-3" toggle={toggleEditModal}>Modifica Ingrediente</ModalHeader>
