...
const handleSaveClick = async (event) => {
  ...
  const corpo = rows.map(row => {
    ...
    return {
      ...
      description: includeDescription ? row.description : row.selectedArticolo.description, // Include description if different
  ...
  try {
    ...
    const response = await apiClient.create('/doceasy/api/fatturavendita/edit', {
      id: id, 
      xml: xmlString,
      sezionale: sezionale,
...
const handleInputChange = (index, field, value) => {
  ...
  document.title = "Modifica Fattura | DgnsDesk";
  ...
  const fetchFattura = async () => {
    if (!id) {
      console.error('No ID provided in location state');
      return;
    }

    try {
      const response = await apiClient.get(`/fattura/${id}`);
      ...
      if (response && response.data) {
        ...
        handleCustomerChange(response.data.cliente)
        setNumero(response.data.numero)
        setSezionale(response.data.sezionale)
        setTotale(response.data.totale)
        handlePagamentiChange(response.data.pagamento)
        setNewDate(response.data.date)
        setRows(response.data.corpo)
  ...
  useEffect(() => {
    fetchFattura();
  }, [id]); // The effect will run when `id` changes
                    ...
                    <Col lg={4}>
                      ...
                      <div>                        
                        ...
                        <Input
                            ...
                            rows="5"
                            readOnly
                            placeholder={`C.F. ${userProfileSession.data.azienda.codiceFiscale} P.Iva ${userProfileSession.data.azienda.partitaIva}
...
${userProfileSession.data.azienda.city}(${userProfileSession.data.azienda.provincia}) IT
                ...
                <CardBody className="p-4">
                  <Row className="g-3">
                  <Col lg={3} sm={6}>
                      <Label for="choices-tipo-doc">Tipo Documento</Label>
                  ...
                  <Col lg={1} sm={6}>
                    ...
                    <Col lg={2} sm={6}>
                    <Label for="choices-sezionale">Sezionale</Label>
                    <div className="input-light">
