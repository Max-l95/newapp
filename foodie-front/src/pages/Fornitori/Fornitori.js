...
import { Card, CardHeader, Container, Row, Col, CardBody, Button, Modal, ModalHeader, ModalBody, ModalFooter, Input, UncontrolledTooltip, Label } from 'reactstrap';
import TableContainer from '../../Components/Common/TableContainerReactTable';
import { APIClient } from '../../helpers/api_helper';
import NewFornitore from './NewFornitore';
...
const Fornitori = () => {
    ...
    const handleEditCategory = async (event) => {
        ...
        try {
            const response = await apiClient.update('/fornitori/edit', {
                id: editId,
    ...
    const confirmMultipleDelete = async () => {
        ...
        try {
            const response = await apiClient.delete('/fornitori/delete', { ids: idsToDelete });
    ...
    document.title = "Fornitori | DgnsDesk";
    ...
    return (
        <React.Fragment>
                ...
                <Container fluid>
                    <Row>
                        <Col lg={12}>
                            <Card>
                                <CardHeader>
                                    <h4 className='card-title mb-0'>Fornitori</h4>
                                </CardHeader>
                                <CardBody>
            ...
            <Modal size="xl" isOpen={editModal} toggle={toggleEditModal} centered>
                <ModalHeader className="bg-light p-3" toggle={toggleEditModal}>Modifica Fornitore</ModalHeader>
...
const SearchTable = ({ 
    ...
    // Toggle select all rows or none
    const handleSelectAll = () => {
        const allIds = data.map(item => item.id);
        onRowSelect({ ids: selectAll ? [] : allIds });
    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            cell: ({ getValue, row }) => {
                const { cognome, nome } = row.original;
                return <span>{getValue() || `${cognome} ${nome}`}</span>;
            },
            enableColumnFilter: false,
            size: "150",
        },
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <div>
                    <Button size="sm" color="primary" onClick={() => onEdit(row.original)}>Modifica</Button>{' '}
                    <Button
                        size="sm"
                        color="danger"
                        onClick={() => {
